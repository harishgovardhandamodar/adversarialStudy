...
def load_and_preprocess_data():
    ...
    # Select a sample of the data for quicker processing
    ...
    df_processed = df_sample[numerical_cols].dropna()
...
def visualize_results(original_data, noisy_data, denoised_data):
    ...
    axes[0, 0].hist(
        original_data["amt"], alpha=0.5, label="Original", bins=50, color="blue"
    )
    axes[0, 0].hist(noisy_data["amt"], alpha=0.5, label="Noisy", bins=50, color="red")
    axes[0, 0].hist(
        denoised_data["amt"], alpha=0.5, label="Denoised", bins=50, color="green"
    )
    ...
    axes[1, 0].scatter(
        noisy_data["lat"], noisy_data["long"], alpha=0.3, label="Noisy", color="red"
    )
