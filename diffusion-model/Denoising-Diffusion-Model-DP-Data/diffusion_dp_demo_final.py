...
def plot_data_comparison(
    original_data, private_data, denoised_data, title="Data Comparison"
):
    ...
    axes[0, 0].hist(
        original_data["amount"], alpha=0.7, label="Original", bins=50, color="blue"
    )
    axes[0, 0].hist(
        private_data["amount"], alpha=0.7, label="Private", bins=50, color="orange"
    )
    axes[0, 0].hist(
        denoised_data["amount"], alpha=0.7, label="Denoised", bins=50, color="green"
    )
    ...
    axes[0, 1].hist(
        original_data["hour"], alpha=0.7, label="Original", bins=24, color="blue"
    )
    axes[0, 1].hist(
        private_data["hour"], alpha=0.7, label="Private", bins=24, color="orange"
    )
    axes[0, 1].hist(
        denoised_data["hour"], alpha=0.7, label="Denoised", bins=24, color="green"
    )
    ...
    axes[0, 2].hist(
        original_data["day_of_week"], alpha=0.7, label="Original", bins=7, color="blue"
    )
    axes[0, 2].hist(
        private_data["day_of_week"], alpha=0.7, label="Private", bins=7, color="orange"
    )
    axes[0, 2].hist(
        denoised_data["day_of_week"], alpha=0.7, label="Denoised", bins=7, color="green"
    )
    ...
    axes[1, 0].bar(
        x - width, merchant_orig.values, width, label="Original", color="blue"
    )
    axes[1, 0].bar(x, merchant_priv.values, width, label="Private", color="orange")
    axes[1, 0].bar(
        x + width, merchant_denoised.values, width, label="Denoised", color="green"
    )
    ...
    axes[1, 1].bar(
        fraud_orig.index, fraud_orig.values, alpha=0.7, label="Original", color="blue"
    )
    axes[1, 1].bar(
        fraud_priv.index, fraud_priv.values, alpha=0.7, label="Private", color="orange"
    )
    axes[1, 1].bar(
        fraud_denoised.index,
        fraud_denoised.values,
        alpha=0.7,
        label="Denoised",
        color="green",
    )
...
def plot_diffusion_process(model, original_data, private_data):
    ...
    with torch.no_grad():
        ...
        denoised_original_scale = scaler.inverse_transform(denooised_values)
...
def main():
    ...
    # Show original vs private distribution comparison
    ...
    axes[1, 0].bar(
        x - width / 2, merchant_orig.values, width, label="Original", color="blue"
    )
    axes[1, 0].bar(
        x + width / 2, merchant_priv.values, width, label="Private", color="orange"
    )
    axes[1, 0].set_title("Merchant Category Distribution")
    axes[1, 0].set_xticks(x)
    axes[1, 0].set_xticklabels(merchant_orig.index, rotation=45)
