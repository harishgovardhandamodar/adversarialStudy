...
parser.add_argument(
    "-c", "--carlini", type=str2bool, default="False", help="Run Carlini attack or not"
)
...
parser.add_argument(
    "-f", "--fgsm", type=str2bool, default="False", help="Run FGSM attack or not"
)

parser.add_argument(
    "-bi", "--bim", type=str2bool, default="False", help="Run BIM attack or not"
)

parser.add_argument(
    "-m", "--mim", type=str2bool, default="False", help="Run MIM attack or not"
)

parser.add_argument(
    "-p", "--pgd", type=str2bool, default="False", help="Run PGD attack or not"
)
...
parser.add_argument("-t", "--times", type=int, required=True, help="Running times")
...
def run_experiment(data_type_mixed: bool, running_times: int):
    ...
    """
    if data_type_mixed:
        dataset = ["adult", "german", "compas", "bank"]
    else:
        dataset = [
            "diabetes",
            "breast_cancer",
    ...
    for dataset_name in dataset:
        ...
        if dataset_name == "adult":
            ...
            dataset_loading_fn = load_bank_df
        else:
        ...
        if RUN_DEEPFOOL:
            ...
            save_datapoints_as_npy(
                "deepfool", dataset_name, deepfool_datapoints, running_times
            )
            deepfool_result_dfs = process_result(deepfool_results, df_info)
            save_result_as_csv(
                "deepfool", dataset_name, deepfool_result_dfs, running_times
            )
        ...
        if RUN_CARLINI:
            ...
            save_datapoints_as_npy(
                "carlini_l_2", dataset_name, carlini_l_2_datapoints, running_times
            )
            carlini_l_2_result_dfs = process_result(carlini_l_2_results, df_info)
            save_result_as_csv(
                "carlini_l_2", dataset_name, carlini_l_2_result_dfs, running_times
            )
            ...
            save_datapoints_as_npy(
                "carlini_l_inf", dataset_name, carlini_l_inf_datapoints, running_times
            )
            carlini_l_inf_result_dfs = process_result(carlini_l_inf_results, df_info)
            save_result_as_csv(
                "carlini_l_inf", dataset_name, carlini_l_inf_result_dfs, running_times
            )
        ...
        if not data_type_mixed and RUN_LOWPROFOOL:
            ...
            save_datapoints_as_npy(
                "lowprofool_l_2", dataset_name, lowprofool_l_2_datapoints, running_times
            )
            lowprofool_l_2_result_dfs = process_result(lowprofool_l_2_results, df_info)
            save_result_as_csv(
                "lowprofool_l_2", dataset_name, lowprofool_l_2_result_dfs, running_times
            )
            ...
            save_datapoints_as_npy(
                "lowprofool_l_inf",
                dataset_name,
                lowprofool_l_inf_datapoints,
                running_times,
            )
            lowprofool_l_inf_result_dfs = process_result(
                lowprofool_l_inf_results, df_info
            )
            save_result_as_csv(
                "lowprofool_l_inf",
                dataset_name,
                lowprofool_l_inf_result_dfs,
                running_times,
            )
        ...
        if RUN_FGSM:
            ...
            save_datapoints_as_npy(
                "fgsm_l_1", dataset_name, fgsm_l_1_datapoints, running_times
            )
            fgsm_l_1_result_dfs = process_result(fgsm_l_1_results, df_info)
            save_result_as_csv(
                "fgsm_l_1", dataset_name, fgsm_l_1_result_dfs, running_times
            )
            ...
            save_datapoints_as_npy(
                "fgsm_l_2", dataset_name, fgsm_l_2_datapoints, running_times
            )
            fgsm_l_2_result_dfs = process_result(fgsm_l_2_results, df_info)
            save_result_as_csv(
                "fgsm_l_2", dataset_name, fgsm_l_2_result_dfs, running_times
            )
            ...
            save_datapoints_as_npy(
                "fgsm_l_inf", dataset_name, fgsm_l_inf_datapoints, running_times
            )
            fgsm_l_inf_result_dfs = process_result(fgsm_l_inf_results, df_info)
            save_result_as_csv(
                "fgsm_l_inf", dataset_name, fgsm_l_inf_result_dfs, running_times
            )
        ...
        if RUN_PGD:
            ...
            save_datapoints_as_npy(
                "pgd_l_1", dataset_name, pgd_l_1_datapoints, running_times
            )
            pgd_l_1_result_dfs = process_result(pgd_l_1_results, df_info)
            save_result_as_csv(
                "pgd_l_1", dataset_name, pgd_l_1_result_dfs, running_times
            )
            ...
            save_datapoints_as_npy(
                "pgd_l_2", dataset_name, pgd_l_2_datapoints, running_times
            )
            pgd_l_2_result_dfs = process_result(pgd_l_2_results, df_info)
            save_result_as_csv(
                "pgd_l_2", dataset_name, pgd_l_2_result_dfs, running_times
            )
            ...
            save_datapoints_as_npy(
                "pgd_l_inf", dataset_name, pgd_l_inf_datapoints, running_times
            )
            pgd_l_inf_result_dfs = process_result(pgd_l_inf_results, df_info)
            save_result_as_csv(
                "pgd_l_inf", dataset_name, pgd_l_inf_result_dfs, running_times
            )
        ...
        if RUN_BOUNDARY:
            ...
            save_datapoints_as_npy(
                "boundary", dataset_name, boundary_datapoints, running_times
            )
            boundary_result_dfs = process_result(boundary_results, df_info)
            save_result_as_csv(
                "boundary", dataset_name, boundary_result_dfs, running_times
            )
        ...
        if RUN_HOPSKIPJUMP:
            ...
            save_datapoints_as_npy(
                "hopskipjump_l_2",
                dataset_name,
                hopskipjump_l_2_datapoints,
                running_times,
            )
            hopskipjump_l_2_result_dfs = process_result(
                hopskipjump_l_2_results, df_info
            )
            save_result_as_csv(
                "hopskipjump_l_2",
                dataset_name,
                hopskipjump_l_2_result_dfs,
                running_times,
            )
            ...
            save_datapoints_as_npy(
                "hopskipjump_l_inf",
                dataset_name,
                hopskipjump_l_inf_datapoints,
                running_times,
            )
            hopskipjump_l_inf_result_dfs = process_result(
                hopskipjump_l_inf_results, df_info
            )
            save_result_as_csv(
                "hopskipjump_l_inf",
                dataset_name,
                hopskipjump_l_inf_result_dfs,
                running_times,
            )
