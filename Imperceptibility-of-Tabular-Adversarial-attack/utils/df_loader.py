...
def load_breast_cancer_df():
    ...
    df = pd.read_csv(
        "./datasets/breast_cancer.csv", delimiter=",", skipinitialspace=True
    )
...
def load_diabetes_df():
    ...
    df = pd.read_csv("./datasets/diabetes.csv", delimiter=",", skipinitialspace=True)
...
def load_bank_df():
    """
    Loads the bank transaction dataset
    """
    target_name = "Fraud Flag"

    df = pd.read_csv(
        "./bank_transaction_data_large.csv", delimiter=",", skipinitialspace=True
    )

    # Convert Fraud Flag column to binary: Y for fraud, N for non-fraud
    df[target_name] = df[target_name].apply(lambda x: "Y" if x == True else "N")

    # Remove transaction ID since it's not a meaningful feature
